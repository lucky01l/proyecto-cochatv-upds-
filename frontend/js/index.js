...
const heroBanner = function ({ results: movieList }) {
  ...
  for (const [index, movie] of movieList.entries()) {
    ...
    document.title = `CochaTV — ${movieList[0].title}`; // el título del portal cambia mediante JS
...
const createMovieList = function ({ results: movieList }, title) {
  ...
  for (const movie of movieList) {
    const movieCard = createMovieCard(movie, genreList); // called from movie_card.js
...
search();
// ===== CochaTV: Reto 4 — manipulación del DOM =====
const mensajes = [
  "¿Qué quieres ver hoy?",
  "Descubre algo nuevo esta noche.",
  "3 títulos nuevos esta semana 👀",
...
btnSorpresa.addEventListener("click", function () {
  ...
  mensajeBienvenida.textContent = aleatorio; // mensaje de bienvenida cambia al hacer clic
});
...
btnInfo.addEventListener("click", function () {
  panelInfo.classList.toggle("oculto"); // el panel aparece/desaparece
});

// ===== CochaTV: mostrar favoritos guardados en la BD (flujo GET) =====
function mostrarFavoritos() {
  fetch("/cochatv/api/favoritos.php")
    .then((respuesta) => respuesta.json())
    .then((favoritos) => {
      const seccion = document.createElement("section");
      seccion.classList.add("movie-list");
      seccion.ariaLabel = "Mis Favoritos";

      seccion.innerHTML = `
        <div class="title-wrapper">
          <h3 class="title-large">★ Mis Favoritos</h3>
        </div>
        <div class="slider-list">
          <div class="slider-inner"></div>
        </div>
      ...
      const contenedor = seccion.querySelector(".slider-inner");

      if (favoritos.length === 0) {
        contenedor.innerHTML = `<p style="color: var(--text-color)">Todavía no agregaste ninguna película a favoritos.</p>`;
      } else {
        for (const fav of favoritos) {
          ...
          tarjeta.innerHTML = `
            <figure class="poster-box card-banner">
              <img src="${imageBaseURL}w342${fav.poster}" alt="${fav.titulo}" class="img-cover" loading="lazy">
            </figure>
            <h4 class="title">${fav.titulo}</h4>
            <p class="genre-text">Agregado: ${fav.fecha_agregado}</p>
          `;
          contenedor.appendChild(tarjeta);
        }
      }

      pageContent.appendChild(seccion);
    })
